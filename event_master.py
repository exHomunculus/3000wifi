...
try:
    ...
    raise Exception('Please call on script in the following manner: python <script> <serial> <ac>')
...
def stopped():
    log(timestamp() + " - Seismo stopped commicating for some reason.")
...
def wakeup():
    ...
    log(timestamp() + " - Waiting for seismo to acknowledge...")
    timeout = 0
    while(a.find('ATE0') == -1):
        if(timeout > maxtimeouts):
            log(timestamp() + " - Seismo not responding. It could be busy, bad battery, or bad comm setup.")
            return 0
        else:
            timeout += 1
            toggleDTR()
...
def prompt():
    ...
    while(a.find('>') == -1):
        if(timeout > maxtimeouts):
            ...
            timeout += 1
            log(timestamp() + " -  Trying to get a prompt " + str(timeout) + " out of "
                + str(maxtimeouts))
            time.sleep(10)
            ser.write('\r')
            a = ser.read(ser.inWaiting())
...
def check():
    log(timestamp() + " - Checking seismo for new data...")
    ser.write('inf\r')
    time.sleep(2)
    a = ser.read(ser.inWaiting())
    if(a.find('[05.') != -1):
        ...
        b = a.split("(")
        c = b[1].split(",")
        log(timestamp() + " - " + c[0] + " values found.")
        if(int(c[0]) > 1):
            return int(c[0])          #return how many
        else:
            log(timestamp() + " -  Just a template.")
            return 1                #return just 1, check was successful but only the template is on seismo
    return 0                        #lost comms?
...
def login():
    log(timestamp() + " - Logging into seismo...")
    ser.write(logincmd)
    time.sleep(2)
    a = ser.read(ser.inWaiting())
    if(a.find('OK') != -1):
...
def is_running():
    global running
    log(timestamp() + " - Checking to see if seismo is currently scanning...")
    ser.write("lst\r")
    time.sleep(5)
    a = ser.read(ser.inWaiting())
    if(a.find('NOT STOPPED') != -1):
        log(timestamp() + " - Currently in scan mode. Halting...")
        running = 1
        b = prompt()
        if(b):
            ser.write("hlt\r\r")
            ...
            if(c.find('OK') != -1):
                log(timestamp() + " - Temporarily halted.")
                return 1
        else:
            stopped()
            return 0
        ...
        running = 0
        log(timestamp() + " - Not in scan mode.")
        return 1
...
def dump():
    log(timestamp() + " - Transferring files...")
    try:
    ...
    time.sleep(5)
    log(timestamp() + " - Transfer complete!")
    a = prompt()
    if(a):
        log(timestamp() + " - Clearing events downloaded from seismo..")
        ser.write("clr E\r")
        time.sleep(2)
        b = ser.read(ser.inWaiting())
        if (b.find('OK') != -1):
            log(timestamp() + " - Events cleared.")
    else:
        stopped()
...
def run():
    if(running == 1):
        log(timestamp() + " - Returning unit back to scan mode...")
        ser.write("run\r\r")
        time.sleep(5)
        b = ser.read(ser.inWaiting())
        if(b.find('OK') == -1):
...
def end():
    log(timestamp() + " - Ending communications with seismo.")
